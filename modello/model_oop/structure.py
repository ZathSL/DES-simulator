            ...
            if len(self.hospitalization_waiting) > 0:
                ...
                if not patient.visited_already:
                    hospitalization_type = \
                        self.simulation.hospitalization_type_distributions[patient.mdc][self.code].sample()
                    if hospitalization_type == "DS":
                        patient.hospitalization_type = "DS"
                        patient.ds += \
                            self.simulation.accesses_per_hospitalization_DS_distribution[patient.mdc][self.code]
                    if hospitalization_type == "DH":
                        patient.hospitalization_type = "DH"
                        patient.dh += \
                            self.simulation.accesses_per_hospitalization_DH_distribution[patient.mdc][self.code]
